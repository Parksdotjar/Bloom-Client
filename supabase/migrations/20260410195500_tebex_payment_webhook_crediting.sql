...
create table if not exists public.commerce_tebex_events (
  id uuid primary key default gen_random_uuid(),
  tebex_webhook_id text not null unique,
...
alter table public.commerce_tebex_events enable row level security;

create or replace function public.commerce_process_tebex_payment_event(
  p_tebex_webhook_id text,
  p_tebex_transaction_id text default null,
  p_email text default null,
  p_package_slug text default null,
  p_tebex_package_id text default null,
...
as $$
declare
  v_webhook_id text := btrim(coalesce(p_tebex_webhook_id, ''));
  v_transaction_id text := nullif(btrim(coalesce(p_tebex_transaction_id, '')), '');
  v_email text := lower(btrim(coalesce(p_email, '')));
  v_slug text := lower(btrim(coalesce(p_package_slug, '')));
  v_tebex_package_id text := nullif(btrim(coalesce(p_tebex_package_id, '')), '');
  v_target_user_id uuid := p_user_id;
  v_existing public.commerce_tebex_events;
  v_event public.commerce_tebex_events;
  v_pack public.commerce_currency_packs;
  v_balance integer;
  v_email_user_ids uuid[];
  v_email_user_count integer := 0;
begin
  if v_webhook_id = '' then
    raise exception 'tebex_webhook_id_required';
  end if;

  select * into v_existing
  from public.commerce_tebex_events
  where tebex_webhook_id = v_webhook_id
  limit 1;
  if found then
    return query
    select
  ...
  insert into public.commerce_tebex_events (
    tebex_webhook_id,
  ...
  if not found and v_tebex_package_id is not null then
    select * into v_pack
    from public.commerce_currency_packs
    where tebex_package_id = v_tebex_package_id and is_active = true
    limit 1;
  end if;

  if not found then
    update public.commerce_tebex_events
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'tebex_webhook_id', v_webhook_id,
      'tebex_transaction_id', v_transaction_id,
      'tebex_package_id', coalesce(v_tebex_package_id, nullif(v_pack.tebex_package_id, ''))
    )
  );
...
grant execute on function public.commerce_process_tebex_payment_event(text, text, text, text, text, uuid, jsonb) to service_role;
