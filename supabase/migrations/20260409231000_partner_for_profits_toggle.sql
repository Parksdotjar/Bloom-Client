alter table public.commerce_capes
  add column if not exists partner_for_profits boolean not null default false;

create index if not exists commerce_capes_partner_for_profits_idx
  on public.commerce_capes (partner_for_profits)
  where partner_for_profits = true;
...
as $$
  ...
  where c.partner_for_profits = true
