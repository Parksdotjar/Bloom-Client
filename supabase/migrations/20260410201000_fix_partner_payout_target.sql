...
begin
  ...
  -- Credit ONLY the explicitly mapped partner user for this cape, never the buyer.
  select m.partner_user_id
    into v_partner_user_id
  from public.commerce_partner_cape_mappings m
  join public.commerce_profiles p on p.user_id = m.partner_user_id
  where m.cape_id = v_cape.id
    and m.is_active = true
    and p.role = 'partner'
  limit 1;

  if v_partner_user_id is not null and v_partner_user_id <> v_user_id then
    ...
    if v_partner_share > 0 then
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'mapped_partner_user_id', v_partner_user_id,
          'cape_id', v_cape.id,
